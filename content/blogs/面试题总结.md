...
- **preload** 加载当前页面的关键资源，提前加载，优先级高，浏览器会优先处理，一般是首屏需要的资源，字体，图片，js，css
- **prefetch** 加载未来页面可能需要的资源，优先级低，等 dom 加载完成后，空闲时加载
- **preconnect** 不加载资源，只提前进行 DNS 解析，TCP 连接
...
步骤 1: 执行同步代码，遇到异步任务执行，然后把对应的回调注册到微任务队列或者宏任务队列中，当主线程执行完成，为空时，从微任务队列依次拿任务到主线程执行（先宏后微，因为第一次的宏任务是 script 标签的解析），若执行期间再次产生微任务，那么放入当前微任务队列结尾执行，若产生宏任务，那么加入到宏任务队列，后续排队执行。直到当前微任务执行完成

步骤 2: 若需要 ui 渲染，那么进行渲染 ui，没有就略过

至此，若已经完成一次事件循环，主线程 => script 标签宏任务 => 微任务 => ui 渲染，微任务是依附在宏任务上的，微任务队列是全局唯一的，宏任务队列是多个的，根据优先级不同，取任务时先取高优先级的宏任务队列中的任务

步骤 3: 再从宏任务队列（根据优先级从不同队列取任务）取出宏任务拿到主线程执行，这期间宏任务的期间，如果再次产生微任务，那么放入全局唯一的微任务队列中，如果产生宏任务，放到对应宏任务队列中
...
- **dns 解析**：按照这个步骤查询，找到 ip，立即中断返回给客户端（浏览器缓存 => 操作系统缓存(host) => 路由器缓存 => 本地服务器缓存 => isp dns 服务器查询 => 根 dns 服务器）
- **tcp 三次握手**
...
> npm、cnpm、pnpm 和 yarn 都是 JavaScript 生态中用于管理依赖包的工具

- npm: 官方默认、在 npm3+ 支持了扁平化依赖树，减少重复依赖安装，兼容性好
- yarn: 2016 年由 Facebook 推出，引入了 yarn.lock 文件（后被 npm 借鉴），和并行安装机制（多个包同时发起安装请求），解决早期 npm 安装速度问题
- pnpm：2017 开源项目，特点：全局安装：所有包被安装到全局存储区，项目中通过链接引用，避免重复存储，生态略差
- cnpm：淘宝团队推出的 npm 镜像客户端，解决官方源在国内安装慢问题，本质是 npm 的 “镜像代理”，命令用法与 npm 完全一致
