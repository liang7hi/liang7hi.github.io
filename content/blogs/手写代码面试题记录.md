...
function f(n) {
  if (n <= 2) return n === 1 ? [1] : [1, 2];
  let temp = Array.from({ length: n }, (_, i) => i + 1); // 生成1~n的数组
  let count = 0; // 报数计数器（从0开始，每次+1后判断是否等于3）
  let index = 0; // 当前报数的人索引

  while (temp.length > 2) {
    count++;
    if (count === 3) {
      temp.splice(index, 1); // 淘汰当前索引的人
      count = 0; // 重置报数
    } else {
      index++; // 报数不是3，下一个人
    }
...
let quickSort = function (arr) {
  ...
  let point = arr.pop(); // 选基准：取数组最后一个元素

  // 2. 分左右：小于基准的放 left，大于等于基准的放 right
  const left = [];
  const right = [];
